...
🧊 **Spin it in 3D** — drag to orbit, scroll to zoom, slide to **explode**, click a part to **isolate** it, and toggle the **reference head** on to preview the worn fit. This is the live model, developed in the open at [github.com/makerphones/daily-driver](https://github.com/makerphones/daily-driver). **Want every part to print?** [See the full parts list →](/learn/daily-driver-parts/)
...
:::note[Reconciliation in progress (2026-06-27)]
The **live 3D model above** and the repo's `DESIGN-LOG.md`, `BOM.md`, and `PARTS.md` are the
authoritative current state. Some prose below predates recent decisions — the headband is now a
sprung **Beyerdynamic metal bow**, the cup↔yoke joint a **shoulder-screw pivot**, the earcup a
**one-piece** shell, and the driver a **Kingstate 40 mm** — a full spec refresh follows the
current readiness pass (acoustics + headband).
:::
...
- **Acoustic volumes** (computed from the geometry, so they track the design — full breakdown in the
  repo's generated `ACOUSTICS.md`):
  - Rear (cup interior): **~143 cc** (⌀78 × 30 mm — the earlier "~90 cc" was an arithmetic slip)
  - Front (ear ↔ baffle): **~68 cc** (pad opening ⌀60 × 24 mm pad depth — the main tuning lever)
  - Total enclosed: **~211 cc**. Parameterized for easy adjustment (the reasoning lives in
    [acoustic chambers and enclosures](/learn/acoustic-chambers-and-enclosures/)).
- **Open-back vent**: ring of 6–8 oval slots on the rear face, ~40% open area.
- **Pad attachment**: a DT770-style retaining lip on the cup's outer rim; the earpad's elastic
  mount-ring stretches over it. **Default pads: Beyerdynamic DT 770/880/990 family** (Dekoni and
  Brainwavz pads are interchangeable premium/tuning swaps).
