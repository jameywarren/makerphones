...
import ConnectorWiring from '../../../components/diagrams/ConnectorWiring.astro';

You can have the perfect drivers, the ideal acoustic chamber, and beautifully tuned damping - but if a cable breaks or a connector fails, your headphones are useless. I learned this the hard way at HeadRoom when we'd see beautifully sounding headphones come back for repair because of a $0.50 connector that failed. This chapter covers which cables, connectors, and hardware actually matter.
...
<Figure label="Fig. 1" caption="TRS and TRRS contact layout: tip is always left; the conductor map is the wiring step builders most often get wrong.">
  <ConnectorWiring />
</Figure>
...
Very secure locking connection, professional appearance, takes abuse without failing. More expensive ($5-15 per connector), requires specific housing design, less universal cable availability. Best for serious builds where durability matters most.
...
With the components covered, it's time to put them together into an actual build. Start with [Design Methodology](/learn/design-methodology/), the complete workflow from concept to finished headphones, then continue through 3D design, chamber design, assembly, and damping strategy.
