...
Walk into any audio store and you'll see headphones in all shapes and sizes: massive over-ears, compact on-ears, tiny earbuds. To a listener that's a comfort or portability call. To a builder, the form factor drives everything downstream: acoustic design, driver selection, comfort engineering, and build complexity. Here's what each of the main categories means for DIY projects.
...
Most DIY headphone building happens here, and the reasons stack up fast.
...
:::caution[Designing the cup before measuring the actual driver]
Not all `40mm` drivers are exactly 40mm. Flange diameter, mounting depth, and overall dimensions vary between manufacturers. Order your drivers first, measure the real parts, then design your cup around them, not around the nominal spec.
:::

:::caution[Choosing a 50mm driver assuming more bass]
The reasoning sounds right, bigger driver, more bass. But in a DIY build, bass depends far more on enclosure design and tuning than on driver diameter. The extra bass from `50mm` is real but smaller than you'd expect, and on a first build it's not worth the heavier cups and thinner sourcing options.
:::

## What's Next

Form factor sets the stage; the driver is what actually makes sound. In [Driver Technologies](/learn/driver-technologies/), we'll cover dynamic, planar magnetic, and other driver types, and explain why dynamic drivers are the right choice for most DIY builders.
